...
insert into phieusudungdichvu (sophieu, ngayxuatphieu, manhanvien, mabenhnhan) values ('SP000037', to_date('2024-10-20', 'YYYY-MM-DD'), 'NV011', 'BN000037');
insert into phieusudungdichvu (sophieu, ngayxuatphieu, manhanvien, mabenhnhan) values ('SP000038', to_date('2024-11-05', 'YYYY-MM-DD'), 'NV013', 'BN000038');
insert into phieusudungdichvu (sophieu, ngayxuatphieu, manhanvien, mabenhnhan) values ('SP000039', to_date('2024-11-10', 'YYYY-MM-DD'), 'NV016', 'BN000039');
insert into phieusudungdichvu (sophieu, ngayxuatphieu, manhanvien, mabenhnhan) values ('SP000040', to_date('2024-11-15', 'YYYY-MM-DD'), 'NV018', 'BN000040');
insert into phieusudungdichvu (sophieu, ngayxuatphieu, manhanvien, mabenhnhan) values ('SP000041', to_date('2024-11-20', 'YYYY-MM-DD'), 'NV019', 'BN000041');
insert into phieusudungdichvu (sophieu, ngayxuatphieu, manhanvien, mabenhnhan) values ('SP000042', to_date('2024-11-16', 'YYYY-MM-DD'), 'NV002', 'BN000042');
...
--Update machuyenvien trong bang GiayNhapVien
update giaynhapvien
set machuyenvien='MCV0001' where manhapvien='MNV0005'

update giaynhapvien
set machuyenvien='MCV0002' where manhapvien='MNV0009'

update giaynhapvien
set machuyenvien='MCV0003' where manhapvien='MNV0015'

update giaynhapvien
set machuyenvien='MCV0004' where manhapvien='MNV0017'

update giaynhapvien
set machuyenvien='MCV0005' where manhapvien='MNV0018'
...
CHECK (gioitinh IN ('nam', 'nu', 'NAM', 'NU', 'Nam', 'Nu'));

--NhanVienYTe
ALTER TABLE NhanVienYTe
ADD CONSTRAINT chk_gioitinhyt
CHECK (gioitinh IN ('nam', 'nu', 'NAM', 'NU', 'Nam', 'Nu'));

--Thuoc
ALTER TABLE Thuoc
ADD CONSTRAINT chk_ngaynhap_hansudung
CHECK (ngaynhap < hansudung);

--TheBHYT
ALTER TABLE TheBHYT
ADD CONSTRAINT chk_ngay_hethan_ngaycap
CHECK (NgayHetHan > NgayCap);

--DichVu
ALTER TABLE DichVu
ADD CONSTRAINT check_DonGia
CHECK (DonGia > 0);

--PhongBenh
ALTER TABLE PhongBenh
ADD CONSTRAINT check_SoLuongGiuong
CHECK (SoLuongGiuong > 0 AND SoLuongGiuong <= 10);

--GiuongBenh
ALTER TABLE GiuongBenh
MODIFY TinhTrang VARCHAR2(15) DEFAULT 'Trong' NOT NULL;
...
--Trigger cho ngaycap <= sysdate
CREATE OR REPLACE TRIGGER trg_check_ngaycap
BEFORE INSERT OR UPDATE ON TheBHYT
FOR EACH ROW
BEGIN
    IF :NEW.NgayCap > SYSDATE THEN
        RAISE_APPLICATION_ERROR(-20002, 'Ngày c?p không ???c v??t quá ngày hi?n t?i.');
    END IF;
END;

--Trigger trong machuyenvien
CREATE OR REPLACE TRIGGER trg_update_MaChuyenVien
AFTER INSERT ON GiayChuyenVien
FOR EACH ROW
BEGIN
    -- C?p nh?t MaChuyenVien trong b?ng GiayNhapVien khi có b?n ghi m?i trong GiayChuyenVien
    UPDATE GiayNhapVien
    SET MaChuyenVien = :NEW.MaChuyenVien
    WHERE MaNhapVien = :NEW.MaNhapVien;
END;
/

--Trigger ghi log
CREATE TABLE BenhNhan_Log (
    ID NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,  -- Thêm c?t ID t? ??ng t?ng
    HanhDong VARCHAR2(10),
    MaBenhNhan varchar2(20),
    HoTen varchar2(60),
    NgaySua DATE,
...
CREATE OR REPLACE TRIGGER BenhNhan_Audit_Trigger
AFTER INSERT OR UPDATE OR DELETE ON BenhNhan
FOR EACH ROW
DECLARE
    v_hanhdong VARCHAR2(10);
BEGIN
    IF INSERTING THEN
        v_hanhdong := 'INSERT';
        INSERT INTO BenhNhan_Log (HanhDong, MaBenhNhan, HoTen, NgaySua, Nguoi_thuc_hien)
        VALUES (v_hanhdong, :NEW.MaBenhNhan, :NEW.HoTen, SYSDATE, USER);
    ELSIF UPDATING THEN
        v_hanhdong := 'UPDATE';
        INSERT INTO BenhNhan_Log (HanhDong, MaBenhNhan, HoTen, NgaySua, Nguoi_thuc_hien)
        VALUES (v_hanhdong, :NEW.MaBenhNhan, :NEW.HoTen, SYSDATE, USER);
    ELSIF DELETING THEN
        v_hanhdong := 'DELETE';
        INSERT INTO BenhNhan_Log (HanhDong, MaBenhNhan, HoTen, NgaySua, Nguoi_thuc_hien)
        VALUES (v_hanhdong, :OLD.MaBenhNhan, :OLD.HoTen, SYSDATE, USER);
    END IF;
END;


--Tao index
create index i_benhnhan
on benhnhan (mabenhnhan, hoten)

create index i_benhly
on benhly (mabenh, tenbenh)

create index i_csyt
on cosoyte (macsyt, tencsyt)

create index i_thuoc
on thuoc (mathuoc, tenthuoc)

create index i_nhanvienyte
on nhanvienyte (manhanvien, hoten, chucvu)

create index i_thebhyt
on thebhyt (sothebhyt, mabenhnhan)

create index i_giaynhapvien
on giaynhapvien (mabenhnhan, manhapvien, machuyenvien)

create index i_dichvu
on dichvu (madichvu, tendichvu)

create index i_donthuoc
on donthuoc (madon, manhanvien, mabenhnhan)

create index i_phongbenh
on phongbenh (sophong, makhoa)

create index i_giuongbenh
on giuongbenh (sogiuong, sophong)

create index i_giayravien 
on giayravien (sogiay, mabenhnhan)

create index i_sudungdichvu
on sudungdichvu (sophieu, madichvu)

create index i_sudungthuoc
on sudungthuoc (madonthuoc, mathuoc)

create index i_phieukham
on phieukham (sophieu, mabenhnhan)

create index i_phieusudungdichvu
on phieusudungdichvu (sophieu, mabenhnhan)

create index i_giaychuyenvien
on giaychuyenvien (machuyenvien, mabenhnhan, manhapvien)

--Tao View
--View thong tin benh nhan nhap vien
CREATE OR REPLACE VIEW ThongTinBenhNhanNhapVien AS
SELECT 
    bn.MaBenhNhan,
    bn.HoTen,
    bn.NgaySinh,
...
FROM 
    BenhNhan bn
JOIN 
    GiayNhapVien gnv ON bn.MaBenhNhan = gnv.MaBenhNhan
JOIN 
    BenhLy bl ON gnv.MaBenh = bl.MaBenh
JOIN 
    Khoa k ON gnv.MaKhoa = k.MaKhoa
JOIN 
    PhongBenh pb ON gnv.SoPhong = pb.SoPhong;

select * from thongtinbenhnhannhapvien

--View
CREATE OR REPLACE VIEW SuDungDichVuThuoc AS
SELECT 
    bn.MaBenhNhan,
    bn.HoTen,
    dvc.MaDichVu,
    dvc.TenDichVu,
    sddv.SoLan AS SoLanSuDung,
    dt.MaDon AS MaDonThuoc,
    t.TenThuoc,
    sdt.CachDung,
    sdt.LieuLuong
FROM 
    BenhNhan bn
LEFT JOIN 
    PhieuSuDungDichVu psdv ON bn.MaBenhNhan = psdv.MaBenhNhan
LEFT JOIN 
    SuDungDichVu sddv ON psdv.SoPhieu = sddv.SoPhieu
LEFT JOIN 
    DichVu dvc ON sddv.MaDichVu = dvc.MaDichVu
LEFT JOIN 
    DonThuoc dt ON bn.MaBenhNhan = dt.MaBenhNhan
LEFT JOIN 
    SuDungThuoc sdt ON dt.MaDon = sdt.MaDonThuoc
LEFT JOIN 
    Thuoc t ON sdt.MaThuoc = t.MaThuoc;

select * from sudungdichvuthuoc